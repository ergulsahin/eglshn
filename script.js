document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Gelen veriye göre tabloyu oluşturan fonksiyon.
     * @param {Array} data - Tabloda gösterilecek kuyu verileri.
     */
    function renderTable(data) {
        // İstatistikler her zaman tüm veritabanı üzerinden hesaplanır.
    ...
    // --- FORM GÖNDERME İŞLEMİ (GÜNCELLENDİ) ---
    addWellForm.addEventListener('submit', (event) => {
        event.preventDefault();
        
        const formData = new FormData(addWellForm);
        
        // Değerleri alırken .trim() ile başındaki/sonundaki boşlukları temizliyoruz.
        const kuyu_no = formData.get('kuyu_no').trim();
        const tarih = formData.get('tarih').trim();
        const foraj = formData.get('foraj').trim();
        const durum = formData.get('durum');
        const alinan_foraj = formData.get('alinan_foraj').trim();

        // --- GELİŞTİRİLMİŞ DOĞRULAMA (VALIDATION) ---
        // Hataları biriktirmek için boş bir dizi oluşturuyoruz.
        const errors = [];
        if (!kuyu_no) {
            errors.push("Kuyu Numarası");
        }
        if (!tarih) {
            errors.push("Yapılma Tarihi");
        }
        if (!foraj) {
            errors.push("Foraj");
        }
        if (durum === "Eksik" && !alinan_foraj) {
             errors.push("Alınan Foraj (Eksikse bu alan zorunludur)");
        }

        // Eğer errors dizisinde en az bir hata varsa...
        if (errors.length > 0) {
            // Hata mesajını hangi alanların eksik olduğunu belirterek gösteriyoruz.
            alert(`Lütfen şu zorunlu alanları doldurun:\n- ${errors.join('\n- ')}`);
            return; // Fonksiyonu burada durduruyoruz.
        }

        // Her şey yolundaysa, veriyi bota göndermek için hazırlıyoruz.
        const wellData = {
            kuyu_no: kuyu_no,
            tarih: tarih,
            foraj: foraj,
            durum: durum,
            // Eğer "alinan_foraj" boşsa, normal "foraj" değerini kullan
            alinan_foraj: alinan_foraj || foraj 
        };
